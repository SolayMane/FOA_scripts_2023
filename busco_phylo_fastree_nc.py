...
def main():
    ...
    if supertree:
        ...
        if len(four_single_copy) == 0:
            ...
            for busco in four_single_copy:
                ...
                SeqIO.write(busco_seqs, os.path.join("proteins_4", busco + ".fna"), "fasta")
            ...
            for busco in four_single_copy:
                mp_commands.append(
                    [os.path.join("proteins_4", busco + ".fna"), os.path.join("alignments_4", busco + ".aln")])
    ...
    if supermatrix:
        ...
        print_message("Writing nucleotides sequences to: " + os.path.join(working_directory, "nucleotides"))

        for busco in single_copy_buscos:
            busco_seqs = buscos[busco]

            SeqIO.write(busco_seqs, os.path.join(working_directory, "nucleotides", busco + ".fna"), "fasta")

        print("")
        print_message("Aligning nucleotides sequences using MUSCLE with", threads, "threads to: ",
                      os.path.join(working_directory, "alignments"))#to correct path to file where to store

        mp_commands = []

        for busco in single_copy_buscos:
            mp_commands.append([os.path.join(working_directory, "nucleotides", busco + ".fna"),
...
#        os.system("iqtree -s SUPERMATRIX.aln -bb 1000 -alrt 1000 -nt AUTO -ntmax " + str(threads) + " > /dev/null")
        os.system("FastTree -nt -gtr -boot 1000  SUPERMATRIX.aln" + " > SUPERMATRIX.tree")
...
def run_muscle(io):
    os.system("muscle -align " + io[0] + " -output " + io[1] + " > /dev/null 2>&1")#modified options to new version
